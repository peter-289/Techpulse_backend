...
from app.modules.projects.schema.project_schema import ProjectRead
...
def get_service(session: AsyncSession = Depends(get_db)) -> ProjectHubService:
    return ProjectHubService(UnitOfWork(session=session))
